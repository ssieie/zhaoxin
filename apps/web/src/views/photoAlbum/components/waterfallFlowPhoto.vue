...
import { getRandomInt, throttle } from "@blog/utils";
import { Photo, photoApis } from "/@/api/photo.ts";
import PictureViewer from "/@/views/photoAlbum/components/pictureViewer.vue";
...
function createPhotoWrap(arr: Photo[]) {
  ...
  for (let i = 0, len = arr.length; i < len; i++) {
    ...
    if (currentIdx === getColInfo.value.col - 1) {
      let minHCol = Math.min(...Object.values(currentTopContext));
      for (const idx in currentTopContext) {
        if (currentTopContext[idx] === minHCol) {
          currentIdx = +idx;
          break;
        }
      }
...
const pictureViewerRef = ref();
const viewPicturesHandler = (e: Event) => {
  if (["IMG"].includes(e.target!.tagName)) {
    pictureViewerRef.value.viewer({
...
<template>
  <div>
    <div
      ...
      ref="photoRef"
      id="my-gallery"
      @click="viewPicturesHandler($event)"
    ></div>
    <div
      class="waterfall relative h-60px m-t-20px"
      ref="loadMoreRef"
      v-if="more"
    >
      <div></div>
    ...
    <picture-viewer ref="pictureViewerRef" />
  </div>
</template>
